...
File.new(TGUI_CABI_HPP).each_line do |line|
  if lm = /^\s*(C_ABI_(?:RAW|METHOD|STATIC|SETTER|GETTER|TESTER|MAKE|FREE|SIGNAL))\s+(.+)ABI_([[:alnum:]]*)_(\w+)(.*);/.match(line)
    ...
    when "C_ABI_SIGNAL"
      classes[lm[3]] << "def #{method_name}(*a, &b);   Abi.call_arg_map! a; signal = Abi.#{function_name}(@pointer, *a)#{parser}; block_given? ? signal.connect(self, &b) : signal; end"
      classes[lm[3]] << "def #{method_name}=(a);    signal = Abi.#{function_name}(@pointer)#{parser}; signal.connect(self, &a); end"
    end
  end
end
