...
loader_file = File.new("../lib/gen/tgui-abi-loader.gf.rb", "w")
...
File.new(TGUI_CABI_HPP).each_line do |line|
  if lm = /^\s*(C_ABI_(?:RAW|METHOD|STATIC|SETTER|GETTER|TESTER|MAKE|FREE|SIGNAL))\s+(.+)ABI_([[:alnum:]]*)_(\w+)(.*);/.match(line)
    ...
    when "C_ABI_SIGNAL"
      classes[lm[3]] << "def #{method_name}(*a, &b);   Abi.call_arg_map! a; signal = Signal.new pointer: Abi.#{function_name}(@pointer, *a), autofree: false; block_given? ? signal.connect(&b) : signal; end"
      classes[lm[3]] << "def #{method_name}=(a);    signal = Signal.new pointer: Abi.#{function_name}(@pointer), autofree: false; signal.connect(&a); end"
    end
  end
end
...
FileUtils.cp TGUI_DLL, '../ext/tgui.dll'